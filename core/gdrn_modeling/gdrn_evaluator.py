...
def gdrn_inference(cfg, model, data_loader, amp_test=False):
    """Run model on the data_loader and evaluate the metrics with evaluator.
    Also benchmark the inference speed of `model.forward` accurately. The model
    will be used in eval mode.
    ...
    with inference_context(model), torch.no_grad():
        ...
        for idx, inputs in enumerate(data_loader):
            ...
            if not isinstance(inputs, list):  # bs=1
                inputs = [inputs]
            batch = batch_data(cfg, inputs, phase="test")
            if out.get(inputs[0]['file_name'][0].split('/')[-1], None) is None:
                out[inputs[0]['file_name'][0].split('/')[-1]] = []
            ...
            for i in range(len(out_dict['trans'])):
                ele = {}
                ele['class'] = batch['roi_cls'][i].item()
                ele['trans'] = out_dict['trans'][i].cpu().numpy().tolist()
                ele['rot'] = out_dict['rot'][i].cpu().numpy().tolist()
                out[inputs[0]['file_name'][0].split('/')[-1]].append(ele)
