...
class Epose_Dataset(object):
    ...
    def __init__(self, data_cfg):
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.epose_dado.id2obj.items() if obj_name in self.objs]
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(self.models_root, f"obj_{ref.epose_dado.obj2id[obj_name]:06d}.ply"),
...
Epose__OBJECTS = ["dado_m5"]
Epose_OCC_OBJECTS = ["dado_m5"]
################################################################################

SPLITS_EPOSE = dict(
    epose_dado_train=dict(
        name="epose_dado_train",
        dataset_root=osp.join(DATASETS_ROOT, "custom/epose_dado/"),
        models_root=osp.join(DATASETS_ROOT, "custom/epose_dado/models"),
        objs=Epose__OBJECTS,  # selected objects
        ann_files=[
            osp.join(DATASETS_ROOT, "custom/epose_dado/image_set/{}_{}.txt".format(_obj, "train"))
            for _obj in Epose__OBJECTS
        ],
        image_prefixes=[
            osp.join(DATASETS_ROOT, "custom/epose_dado/test/{:06d}".format(ref.epose_dado.obj2id[_obj]))
            for _obj in Epose__OBJECTS
        ],
        xyz_prefixes=[
            osp.join(DATASETS_ROOT, "custom/epose_dado/test/xyz_crop/{:06d}".format(ref.epose_dado.obj2id[_obj]))
            for _obj in Epose__OBJECTS
        ],
    ...
    epose_dado_test=dict(
        name="epose_dado_test",
        dataset_root=osp.join(DATASETS_ROOT, "custom/epose_dado/"),
        models_root=osp.join(DATASETS_ROOT, "custom/epose_dado/models"),
        objs=Epose__OBJECTS,
        ann_files=[
            osp.join(DATASETS_ROOT, "custom/epose_dado/image_set/{}_{}.txt".format(_obj, "test"))
            for _obj in Epose__OBJECTS
        ],
        # NOTE: scene root
        image_prefixes=[
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    MetadataCatalog.get(name).set(
        id="epose_dado",  # NOTE: for pvnet to determine module
