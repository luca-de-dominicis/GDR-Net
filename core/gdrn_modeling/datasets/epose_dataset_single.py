    ...
    def __call__(self):  # Epose_Dataset
        ...
        for ann_file, scene_root, xyz_root in zip(tqdm(self.ann_files), self.image_prefixes, self.xyz_prefixes):
            ...
            for im_id in tqdm(indices):
                ...
                if gt_dict.get(str_im_id, None) is None:
                    continue
...
SPLITS_EPOSE = dict(
    epose_single_train=dict(
        name="epose_single_train",
        dataset_root=osp.join(DATASETS_ROOT, "custom/epose_single/"),
        models_root=osp.join(DATASETS_ROOT, "custom/epose_single/models"),
        objs=Epose__OBJECTS,  # selected objects
        ann_files=[
            osp.join(DATASETS_ROOT, "custom/epose_single/image_set/{}_{}.txt".format(_obj, "train"))
            for _obj in Epose__OBJECTS
        ],
        image_prefixes=[
            osp.join(DATASETS_ROOT, "custom/epose_single/test/{:06d}".format(ref.epose_full.obj2id[_obj]))
            for _obj in Epose__OBJECTS
        ],
        xyz_prefixes=[
            osp.join(DATASETS_ROOT, "custom/epose_single/test/xyz_crop/{:06d}".format(ref.epose_full.obj2id[_obj]))
            for _obj in Epose__OBJECTS
        ],
    ...
    epose_single_test=dict(
        name="epose_single_test",
        dataset_root=osp.join(DATASETS_ROOT, "custom/epose_single/"),
        models_root=osp.join(DATASETS_ROOT, "custom/epose_single/models"),
        objs=Epose__OBJECTS,
        ann_files=[
            osp.join(DATASETS_ROOT, "custom/epose_single/image_set/{}_{}.txt".format(_obj, "test"))
            for _obj in Epose__OBJECTS
        ],
        # NOTE: scene root
        image_prefixes=[
